...
public class Customers {
	...
	public void getCustomer() 
	{
		try {
			   ...
			   res = st.executeQuery("select * from customer order by customer_id");
			   System.out.println("Customer_id\t CustomerName\t City \t\t Contact No");
			   System.out.println("-----------------------------------------------------------");
			  while (res.next()) 
			  {
	...
	public void updateCustomer(int customer_id,String city) 
	{

		try {
			 con = getConnection();
		    ps = con.prepareStatement("update customer"
		    		+ " set City=? where customer_id=?");
		    ps.setString(1, city);
